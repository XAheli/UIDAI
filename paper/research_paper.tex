...
Analysis reveals significant variation across days (Figure 1). Tuesday shows the highest mean enrollment (72.59 per record), while Wednesday shows the lowest (15.39).
...
This counter-intuitive finding suggests weekend enrollment centers may be more efficient or serve areas with different demographic patterns.

\subsection{Geographic Analysis}

\subsubsection{Regional Distribution}

Table \ref{tab:regional_dist} shows the distribution across regions.
...
\textbf{Interpretation:} Low-HDI states show higher enrollment volumes, likely due to larger unregistered populations requiring new Aadhaar enrollments, while high-HDI states have near-complete coverage.
...
Table \ref{tab:rainfall} shows enrollment by rainfall zone.
...
Table \ref{tab:hypothesis} summarizes hypothesis tests.
...
Significant correlations with total enrollment:
\begin{itemize}[noitemsep]
